...
@router.post("/documentos/checkout-offer")
@limiter.limit("30/minute")
async def ingerir_documento_checkout_offer(
    request: Request,
    file: UploadFile = File(...),
    ...
    idempotency_key = (
        f"document.extract:v1:{usuario_atual.id}:"
        f"{empresa_fk}:{evidencia.documento_hash}"
    )
    request_fingerprint = (
        f"document.extract:v1:sha256:{evidencia.documento_hash}"
    )

    try:
        CheckoutOfferGrantUsage(db).consumir(
            user_id=usuario_atual.id,
            empresa_id=empresa_fk,
    ...
    except CheckoutOfferGrantUsageError:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Operacao de checkout indisponivel.",
        ) from None
