...
def client_sem_empresa():
    ...
    usuario.id = 1
    usuario.email = "faturamento-ausente-cpf@test.local"
    usuario.empresas = []
...
def test_cpf_sem_faturamento_bloqueia_sem_chamar_motor(monkeypatch):
    ...
    resultado = assistente_service.responder_cpf(
        "quanto pago de imposto autonomo em 2026"
    )
...
def test_cpf_faturamento_zero_nao_presume_5000(monkeypatch):
    chamada = {}
    ...
    resultado = assistente_service.responder_cpf(
        "quanto pago de imposto autonomo em 2026 "
        "com faturamento de 0 por mes"
    )

    assert chamada["analysis_type"] == "cpf_tax"
...
def test_endpoint_cpf_sem_faturamento_bloqueia_estruturado(
    client_sem_empresa,
):
    resposta = client_sem_empresa.post(
        "/perguntar",
        json={
            "pergunta": "quanto pago de imposto autonomo em 2026"
        },
    )
